...
WITH RECURSIVE management_report AS (
  SELECT
    e.*,
    '' AS "management_chain"
  FROM
  ...
  UNION ALL
    SELECT
      e.*,
      CASE 
        WHEN e1.management_chain = '' THEN FORMAT('%s (%s)', e1.name, e1.id)
        ELSE FORMAT(e1.management_chain || ' -> ' || '%s (%s)', e1.name, e1.id)
      END AS "management_chain"
    FROM
      employees AS e, management_report AS e1
    WHERE
      e.manager_id = e1.id
)
